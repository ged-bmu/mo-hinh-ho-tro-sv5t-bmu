...
useEffect(() => {
  ...
  // =====================================================
useEffect(() => {
  if (!id || !currentAcademicYear?.id) return;
...
async function loadCriteriaResults() {
  if (!id || !currentAcademicYear?.id) return;
  ...
  if (error) {
    console.error("LOAD CRITERIA RESULTS ERROR:", error);
    return;
  }

  if (data) {
    setCriteriaResults(data);
    return;
  }
  ...
  if (insertError) {
    console.error("CREATE CRITERIA RESULTS ERROR:", insertError);
    return;
  }

  setCriteriaResults(newData);
}
  // =====================================================
  // LOAD REPORT THEO NĂM HỌC
  // =====================================================
async function loadReports() {
  ...
  async function updateGhiChu() {
    ...
    if (userError || !user) {
      alert(
        "Không xác định được người duyệt BCH."
      );
      return;
    }
...
async function loadFiles() {
  ...
  setDaoDucFiles(
    files.filter((f) => f.folder === "dao-duc")
  );

  setHocTapFiles(
    files.filter((f) => f.folder === "hoc-tap")
  );

  setTheLucFiles(
    files.filter((f) => f.folder === "the-luc")
  );

  setTinhNguyenFiles(
    files.filter((f) => f.folder === "tinh-nguyen")
  );

  setHoiNhapFiles(
    files.filter((f) => f.folder === "hoi-nhap")
  );

  setUuTienFiles(
    files.filter((f) => f.folder === "uu-tien")
  );

  setBaoCaoFiles(
    files.filter((f) => f.folder === "bao-cao")
  );
}
  ...
  async function deleteFile(
    ...
    try {
      ...
      if (
        ...
        if (!fileId) {
          alert(
            "Không tìm thấy ID file Google Drive"
          );
          return;
        }
          ...
          >
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  await sendNotification(
                    ...
                    "Ban Chủ nhiệm CLB SV5T vừa nhận xét hồ sơ của bạn. Hãy kiểm tra trong mục Quản lí hồ sơ",
                    "/"
                  );
              ...
              style={{
                ...
                padding:
                  "clamp(8px, 0.8vw, 11px) clamp(12px, 1vw, 18px)",
                border: "none",
                borderRadius: "8px",
                background:
                  savingNhanXet
                      ...
                      >
                        ...
                        >
                          ...
                          <input
                            type="checkbox"
                           checked={
  criterion.key === "dao-duc"
    ? criteriaResults?.dao_duc || false
    : criterion.key === "hoc-tap"
    ? criteriaResults?.hoc_tap || false
    : criterion.key === "the-luc"
    ? criteriaResults?.the_luc || false
    : criterion.key === "tinh-nguyen"
    ? criteriaResults?.tinh_nguyen || false
    : criterion.key === "hoi-nhap"
    ? criteriaResults?.hoi_nhap || false
    : false
}
                            onChange={(
                              e
                ...
                >
                  ...
                  >
                    ...
                    <input
                      type="checkbox"
                      checked={
  previewFolder === "dao-duc"
    ? criteriaResults?.dao_duc || false
    : previewFolder === "hoc-tap"
    ? criteriaResults?.hoc_tap || false
    : previewFolder === "the-luc"
    ? criteriaResults?.the_luc || false
    : previewFolder === "tinh-nguyen"
    ? criteriaResults?.tinh_nguyen || false
    : previewFolder === "hoi-nhap"
    ? criteriaResults?.hoi_nhap || false
    : false
}
                      onChange={(e) =>
